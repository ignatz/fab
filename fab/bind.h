...
// adapted from ZTL
template<size_t To, typename NoPack = pack<>, size_t Idx = 0>
struct range;

template<size_t To, size_t Idx, template<typename ...> class Pack, typename ... Args>
...
std::function<Ret (Args...)> bind_helper(
	Ret (Lambda::*f)(Args...) const,
	Lambda const& l,
	Pack<Range...>)
{
...
std::function<Ret (Args...)> bind(Ret (Lambda::*f)(Args...) const, Lambda const& l)
{
